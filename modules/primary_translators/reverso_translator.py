...
class ReversoTranslationService(PrimaryTranslationService):
    def __init__(self, *, logger = None):
        # If no logger passed, use a module‐level logger
        self.logger: logging.Logger = logger or logging.getLogger(__name__)
        self.service_name = "reverso"

    def translate(self, term: str, source_lang: str, target_lang: str) -> str | None:
        try:
            translated = translators.translate_text(term, translator="reverso", from_language=source_lang, to_language=target_lang)
            return(translated)
...
if __name__ == "__main__":
    ...
    translator = ReversoTranslationService()
    term = "hello world"
    source_lang = "en"
    target_lang = "de"
    try:
